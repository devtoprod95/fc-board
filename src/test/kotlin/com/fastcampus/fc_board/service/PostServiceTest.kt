...
class PostServiceTest(
        ...
        When("작상자가 동일하지 않으면") {
            ...
            then("삭제할 수 없는 게시물 입니다 예외가 발생한다.") {
                shouldThrow<PostNotDeletableException> {
                    postService.deletePost(saved2.id, "delete harris")
                }
            }
        }
    }
    given("게시글 상세조회시") {
        ...
        When("정상 조회시") {
            ...
            then("게시글의 내용이 정상적으로 반환됨을 확인한다.") {
                postRepository.findByIdOrNull(saved.id) shouldNotBe null
    ...
    given("게시글 목록 조회시") {
        ...
        When("좋아요가 2개 추가되었을 때") {
            ...
            val likedPage = postService.findPageBy(PageRequest.of(0, 5), PostSearchRequestDto(tag = "tag5"))
